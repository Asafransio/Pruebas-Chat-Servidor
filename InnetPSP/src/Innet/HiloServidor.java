...
public class HiloServidor extends Thread
{
	...
	String[] cadena = {"Azul", "Amarillo", "Verde", "Rojo", "Marron"};
	...
	public void run()
	{
		ChatServer.mensaje.setText("Número de conexiones actuales: " +
				ChatServer.ACTUALES);
		String texto = ChatServer.textarea.getText();
		...
		while(!fin)
		{
			...
			try
			{
				...
				{
					ChatServer.textarea.append(cadena + "\n");
					texto = ChatServer.textarea.getText();
					EnviarMensajes(texto);
				}
			}
			catch (Exception ex)
	...
	{
		for(int i=0; i<ChatServer.CONEXIONES; i++)
		{
			Socket socket = ChatServer.tabla[i];
			try
			{
